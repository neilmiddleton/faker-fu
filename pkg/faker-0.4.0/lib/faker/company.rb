module Faker
  class Company
    ...
    Formats = [
      Proc.new { [ Person.last_name, suffix ].join(' ') },
      Proc.new { [ Person.last_name, Person.last_name ].join('-') },
      Proc.new { "%s, %s and %s" % [ Person.last_name, Person.last_name, Person.last_name ] }
      ]
  end
end