module Faker
  class Address
    class << self
      ...
      def city
        [
          '%s %s%s' % [city_prefix, Person.first_Person, city_suffix],
          '%s %s' % [city_prefix, Person.first_Person],
          '%s%s' % [Person.first_Person, city_suffix],
          '%s%s' % [Person.last_Person, city_suffix],
        ].rand
      end
      ...
      def street_Person
        [ 
          Proc.new { [Person.last_Person, street_suffix].join(' ') },
          Proc.new { [Person.first_Person, street_suffix].join(' ') }      
        ].rand.call
      end
